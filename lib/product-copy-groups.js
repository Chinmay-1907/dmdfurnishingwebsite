...
export const SUBCATEGORY_COPY_GROUPS = {
  ...
  "Writing Desk": {
    ...
    specification: {
      ...
      body: 'A {name} is typically specified for home offices, guest room workstations, or compact office layouts where a full executive desk will not fit the footprint. DMD builds each unit to the leg style, top finish, and dimensions you confirm, so a small order still gets the same shop-drawing process as a large one. Home-office buyers and designers send the room dimensions and finish sample; we return a production timeline before the order is confirmed.',
    },
  },
  "Amenity Tower": {
    materials: {
  ...
  "Dresser": {
    ...
    specification: {
      ...
      body: 'A {name} is typically ordered to match an existing or coordinated bedroom set, so DMD confirms finish and hardware against the nightstand or headboard already specified. Buyers and designers send a finish sample or existing product reference; we return a matched build with drawer configuration confirmed before production. Because each piece is built to order, drawer count and width can be adjusted to the room, and later reorders draw from the same finish records for a true match.',
    },
  },
  "Headboards": {
    materials: {
    ...
    specification: {
      ...
      body: 'A {name} is specified against the bed frame and mattress size already chosen, since headboard width and mounting height depend on that program. Homeowners and designers send the bed width and finish sample; DMD confirms mounting hardware and produces the headboard to match. Concealed mounting hardware and templated install dimensions keep the installation straightforward for a single room or a full renovation, and a wall-mount design means the headboard installs without touching the bed frame itself.',
    },
  },
  "Luggage Bench": {
    materials: {
  ...
  "Office Chair": {
    ...
    specification: {
      heading: 'Specified for the room it actually sits in',
      body: 'A {name} is ordered against the desk height and room finish it pairs with, so homeowners and designers send the desk specification and finish preference before DMD confirms the chair build. Because it ships as a single unit rather than a fleet order, lead time and minimums are lighter than a full office seating program while the underlying construction standard stays the same. If a second matching chair is needed later, the original build specification stays on file.',
    },
  },
  "Outdoor Dining Set": {
    materials: {
    ...
    specification: {
      ...
      body: 'A {name} is specified around the climate it will sit in year-round, not the mild weather of a showroom, so DMD confirms frame and finish ratings against your location before production. Homeowners and designers send the patio layout and expected season length; we recommend material ratings appropriate to that exposure and return a set built to match your indoor furniture scheme if continuity matters to the project, with stacking options available for seasonal storage.',
    },
  },
  ...
  "Sideboard": {
    ...
    specification: {
      ...
      body: 'A {name} is specified against the dining table and room finish it accompanies, so homeowners and designers send the room dimensions and finish sample before DMD confirms door and drawer configuration. Because each piece is built to order, width and storage layout can be adjusted to the specific wall it sits against rather than forcing the room to fit a stock size, and drawer versus open-shelf storage can be mixed within the same unit.',
    },
  },
  "Storage Cabinets": {
    materials: {
  ...
  "Toy Storage": {
    materials: {
      ...
      body: 'A {name} is built on a plywood or MDF carcass faced in {material}, with rounded, sanded edges and soft-close hardware on every cubbie or drawer, because a kids storage piece takes more daily impact than almost any other furniture category in the house. Interior compartments are finished and sized for bins or loose toys, and the frame is anchored with anti-tip hardware as standard given the climbing a toy storage unit invites.',
    },
    specification: {
      heading: 'Sized to the room and the storage system already in place',
      body: 'A {name} is specified against the room dimensions and any existing storage bins or organization system, so DMD confirms cubbie count and compartment sizing before production. Parents and designers send room dimensions and finish preference; we return a configuration sized to the space with anti-tip anchoring details for the installer. Finish can be matched to existing bedroom furniture on request, and unit count scales for classroom or shared-room orders across a full renovation.',
    },
  },
  "Wardrobe": {
    materials: {
    ...
    specification: {
      ...
      body: 'A {name} is specified against the actual room dimensions and door swing clearance, since wardrobe depth and door style depend on the space available. Homeowners and designers send room dimensions and a finish sample; DMD confirms door configuration and interior layout before production. Because each unit is built to order, sliding-door and hinged-door options are both available without a custom-order premium, and interior fittings can be adjusted to the storage mix the room needs.',
    },
  },
};
