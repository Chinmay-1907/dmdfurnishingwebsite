 ...
 *   1. Exact subcategoryName match against the 66 per-subcategory groups in
 *      product-copy-groups.js (one group per distinct XML subcategory,
 *      written by copywriters -- see copy-groups-manifest.json).
 *   2. SUBCATEGORY_GROUP_MAP exact-alias match against the 15 legacy
 *      furniture-family groups below (COPY_GROUPS).
 *   3. resolveByKeyword() fallback into the same 15 legacy groups, for any
 *      subcategory name not covered by #1 or #2 (e.g. future catalog
 *      additions).
 *   4. COPY_GROUPS.default.
 *
 * ProductDetailPage previously rendered two identical boilerplate cards on all
 * 174 product pages. Each group carries two expert blocks:
...
// Case-insensitive lookup for the 66 per-subcategory groups (one entry per
// distinct product subcategoryName, see copy-groups-manifest.json). Built
// once at module load. Keys are lowercased subcategoryName strings so
// getProductCopy's already-lowercased `sub` can look up directly.
const SUBCATEGORY_COPY_GROUPS_BY_LOWER_NAME = Object.fromEntries(
  Object.entries(SUBCATEGORY_COPY_GROUPS).map(([name, group]) => [name.toLowerCase(), group])
);
...
const COPY_GROUPS = {
  ...
  closets: {
    materials: {
      heading: 'How much space does a walk-in closet system need?',
      body: "There is no single minimum: a single-loaded compact system can fit a condo-width closet as narrow as four to five feet, while an L-shaped or double-sided layout needs a room deep enough for a walkway plus storage on one or both sides. DMD builds each system from modular HPL laminate panel components on an engineered-wood carcass, so the wall runs, shelf heights, and drawer bank widths are sized to the actual room rather than a fixed footprint. Panels are edge-banded and moisture-sealed on every exposed surface, hanging rods sit on reinforced brackets rated for daily use, and drawers run on full-extension, soft-close commercial slides so the system holds up under the traffic a primary suite or model unit closet actually gets.",
    },
    specification: {
      heading: 'Can a walk-in closet system be installed in a condo or a smaller unit?',
      body: "Yes. Reduced-depth, single-loaded layouts are built specifically for condo and urban multi-family footprints where a full L-shaped or double-sided system would not clear the door swing or walkway minimums. Property developers, multi-family owners, and interior designers typically specify walk-in closet systems alongside the rest of the unit's casegoods package so finishes match across the bedroom. DMD builds each system to the room dimensions and finish schedule you provide, whether that is a single condo closet or a full amenity-unit rollout, and returns shop drawings for approval before production. Send your room dimensions, unit count, and finish standard and we will quote the layout that fits.",
    },
  },
...
// ---------------------------------------------------------------------------
// Finer-grained copy groups (written later, one per subcategory rather than
// one per furniture-type family) can embed {name}, {material}, {color},
// {subcategory}, {place} tokens in their heading/body text. The 15 groups
// above have no tokens today, so interpolate() is a no-op on their text and
// output stays byte-identical to before this change.
//
// Per-var fallback strings keep a sentence readable when a product has no
// value for that attribute (e.g. no Color spec on the product row).
const VAR_FALLBACKS = {
  name: 'this piece',
  material: 'commercial-grade materials',
  color: 'your specified finish',
  subcategory: 'furniture',
  place: 'commercial spaces',
};

// Spec names that satisfy each var, checked in order. The XML uses two
// specification schemas across the catalog (Material/Finish on ~179 products,
// Materials/Color/Style on ~581 products), so both names are checked.
const SPEC_NAME_ALIASES = {
  material: ['Material', 'Materials'],
  color: ['Color'],
};

function findSpecValue(specifications, names) {
  if (!Array.isArray(specifications)) return '';
  for (const name of names) {
    const match = specifications.find(
      (spec) => spec?.name?.toLowerCase() === name.toLowerCase() && spec.value
    );
...
/**
 * Builds the {name, material, color, subcategory, place} lookup used by
 * interpolate(). `product` is optional; every field degrades to '' (and then
 * to its VAR_FALLBACKS entry at substitution time) when data is missing.
 *
 * @param {object|null} primary - membership record (subcategoryName, placeName)
 * @param {object|null} [product] - full flat product row (name, specifications)
 */
export function buildCopyVars(primary, product) {
  return {
    name: product?.name || '',
    material: findSpecValue(product?.specifications, SPEC_NAME_ALIASES.material),
    color: findSpecValue(product?.specifications, SPEC_NAME_ALIASES.color),
    subcategory: primary?.subcategoryName || '',
    place: primary?.placeName || '',
  };
}

/**
 * Replaces {varName} tokens in `text` with vars[varName], falling back to
 * VAR_FALLBACKS[varName] when the value is missing/empty. An unrecognized
 * token (not in VAR_FALLBACKS) is left untouched rather than silently erased.
 */
export function interpolate(text, vars) {
  ...
  return text.replace(/\{(\w+)\}/g, (token, key) => {
    ...
    return Object.prototype.hasOwnProperty.call(VAR_FALLBACKS, key)
      ? VAR_FALLBACKS[key]
      : token;
  });
}
...
function resolveByKeyword(haystack) {
  ...
  if (/closet|wardrobe|dressing room/.test(haystack)) return 'closets';
 ...
 *
 * `product` is optional and backwards compatible: existing call sites that
 * only pass `primary` keep working exactly as before, because the 15 groups
 * above carry no {var} tokens and interpolate() is a no-op on plain text.
 *
 * @param {object|null} primary - membership record with subcategoryName / furnitureTypeName
 * @param {object|null} [product] - full flat product row (name, specifications), for template vars
 * @returns {{ materials: {heading, body}, specification: {heading, body} }}
 */
export function getProductCopy(primary, product) {
  const sub = (primary?.subcategoryName || '').trim().toLowerCase();
  const ft = (primary?.furnitureTypeName || '').trim().toLowerCase();

  // First priority: exact subcategoryName match against the 66 per-subcategory
  // groups (one per distinct XML subcategory, written by copywriters). This
  // wins over the 15 legacy furniture-family groups below. "Walk-In Closets"
  // was deliberately excluded from that set of 66, so it always falls through
  // to SUBCATEGORY_GROUP_MAP's 'closets' entry and never hits this branch.
  let group = SUBCATEGORY_COPY_GROUPS_BY_LOWER_NAME[sub];
  ...
  if (!group) {
    ...
    group = COPY_GROUPS[groupKey] || COPY_GROUPS.default;
  }

  const vars = buildCopyVars(primary, product);

  return {
    materials: interpolateBlock(group.materials, vars),
    specification: interpolateBlock(group.specification, vars),
  };
}
