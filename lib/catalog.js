...
/**
 * Returns every product as a flat array with full ancestry metadata.
 * Used by the unified catalog page to render all products in a filterable grid.
 */
      ...
      for (const sub of ft.subcategories) {
        for (const product of sub.products) {
          products.push({
            ...
            furnitureTypeName: ft.name,
            furnitureTypeSlug: ft.slug,
            subcategoryName: sub.name,
            subcategorySlug: sub.slug,
            subcategoryKey: `${place.slug}/${ft.slug}/${sub.slug}`,
            href: `/products/${place.slug}/${ft.slug}/${sub.slug}/${product.slug}`,
...
export function getFilterOptions() {
  ...
  for (const place of catalog) {
    let placeProductCount = 0;
    for (const ft of place.furnitureTypes) {
      let ftProductCount = 0;
      for (const sub of ft.subcategories) {
        const subCount = sub.products.length;
        ftProductCount += subCount;
        subcategories.push({
          slug: sub.slug,
          key: `${place.slug}/${ft.slug}/${sub.slug}`,
          name: sub.name,
          count: subCount,
          furnitureTypeSlug: ft.slug,
          placeSlug: place.slug,
      ...
      placeProductCount += ftProductCount;
      furnitureTypes.push({
        slug: ft.slug,
        name: ft.name,
        count: ftProductCount,
        placeSlug: place.slug,
      });
    }
    spaces.push({
