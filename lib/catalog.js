...
function parseXml() {
  ...
  // Strip comments before scanning: the tag regex below would otherwise match
  // tags INSIDE <!-- --> blocks, resurrecting commented-out products.
  const xml = fs.readFileSync(xmlPath, 'utf-8').replace(/<!--[\s\S]*?-->/g, '');
    ...
    switch (tag) {
      ...
      case 'product': {
        ...
        currentProduct = {
          ...
          sku: decodeEntities(getAttr(attrs, 'sku')),
          tier: decodeEntities(getAttr(attrs, 'tier')),
          images: [],
          specifications: [],
          slug,
        };
  ...
  // ---------------------------------------------------------------------------
  // Apply school/university to educational-facilities merge
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Post-pass: rewrite school/university memberships to educational-facilities.
...
/**
 * Deterministic, robot-safe gallery alt text: product name + subcategory + position.
 * Replaces the raw XML alt text, which the SEO audit found duplicated verbatim
 * across every thumbnail in a gallery (findings-images-programmatic.md P1 — e.g.
 * every King Bed Frame thumbnail sharing the identical alt string). Mechanical
 * template only, built from data already on the record — no hand-authored copy.
 */
function buildGalleryAlt(productName, subcategoryName, position) {
  const subject = subcategoryName ? `${productName} — ${subcategoryName}` : productName;
  return position === 1 ? `${subject}, main view` : `${subject}, view ${position}`;
}
...
export function getProductBySlug(productSlug) {
  ...
  const images = (product.images || []).map((image, index) => ({
    ...image,
    alt: buildGalleryAlt(product.name, primary?.subcategoryName, index + 1),
  }));
 ...
 * excluding the product itself. Singleton subcategories would otherwise return
 * zero, so when fewer than 3 siblings exist the remaining slots are filled from
 * the same furniture type, then the same place. Capped at `limit`.
 */
export function getRelatedProducts(productSlug, limit = 6) {
  ...
  const pool = all.filter((p) => p.slug !== productSlug);
  const related = pool.filter((p) => p.subcategoryKey === target.subcategoryKey);

  if (related.length < 3) {
    const seen = new Set(related.map((p) => p.slug));
    const fillFrom = (matches) => {
      for (const p of pool) {
    ...
    fillFrom((p) => p.furnitureTypeSlugs.some((ft) => target.furnitureTypeSlugs.includes(ft)));
    fillFrom((p) => p.placeSlugs.some((pl) => target.placeSlugs.includes(pl)));
  }
...
export function getFilterOptions() {
  ...
  // Price tiers (fixed order). Only surfaced when products carry a tier.
  const TIER_ORDER = ['Essentials', 'Classic', 'Premium', 'Luxury', 'Inquire', 'Custom'];
  const TIER_NAMES = { Inquire: 'Contact for Pricing', Custom: 'Custom Made' };
  const TIER_RANGES = {
  ...
  const tiers = TIER_ORDER.map((t) => ({
    ...
    name: TIER_NAMES[t] || t,
    range: TIER_RANGES[t] || '',
    count: products.filter((p) => p.tier === t).length,
  })).filter((t) => t.count > 0);

  return { spaces, furnitureTypes, subcategories, tiers };
}
