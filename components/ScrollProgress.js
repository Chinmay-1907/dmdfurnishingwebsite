...
function ScrollProgress({ showPercent = true, position = 'top', variant = 'bar', brandText = 'DMD' }) {
  ...
  if (variant === 'circle') {
    ...
    return (
      <div
        className={styles.scrollCircleContainer}
        ...
        tabIndex={0}
      >
        <div className={styles.scrollCircleCenter}>
          {showPercent ? `${Math.round(progress)}%` : ''}
        </div>
        <svg className={styles.scrollCircleSvg} viewBox="0 0 100 100" aria-hidden="true">
          <defs>
            <path id="scroll-circle-path" d="M50,50 m-38,0 a38,38 0 1,1 76,0 a38,38 0 1,1 -76,0" />
          </defs>
          <circle className={styles.scrollCircleRing} cx="50" cy="50" r="42" />
          <g className={styles.scrollCircleText} style={{ transform: `rotate(${ringRotation}deg)` }}>
            <text>
  ...
  return (
    <div className={styles.scrollProgressContainer} style={barStyle} aria-hidden="true">
      <div className={styles.scrollProgressBar} style={{ width: `${progress}%` }} />
      {showPercent && (
        <div className={styles.scrollProgressLabel}>{Math.round(progress)}%</div>
      )}
    </div>
  );
}
